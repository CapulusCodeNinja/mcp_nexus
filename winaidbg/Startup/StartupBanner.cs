...
internal class StartupBanner
{
    ...
    private void DisplayStartupHeader()
    {
        ...
        else
        {
            ...
            var configuredHost = sharedConfiguration.WinAiDbg.Server.Host;
            var configuredPort = sharedConfiguration.WinAiDbg.Server.Port;
        ...
        m_Logger.Info("╔═══════════════════════════════════════════════════════════════════╗");
        m_Logger.Info("                            WinAiDbg STARTUP");
        m_Logger.Info(string.Empty);
    ...
    {
        var host = m_Settings.Get().WinAiDbg.Server.Host ?? "0.0.0.0";
        var port = m_Settings.Get().WinAiDbg.Server.Port;
    ...
    {
        var transportMode = m_Settings.Get().WinAiDbg.Transport.Mode ?? "http";
    ...
    {
        var cdbPath = m_Settings.Get().WinAiDbg.Debugging.CdbPath ?? string.Empty;
        var commandTimeout = m_Settings.Get().WinAiDbg.Debugging.CommandTimeoutMs;
        var symbolRetries = m_Settings.Get().WinAiDbg.Debugging.SymbolServerMaxRetries;
        var symbolPath = m_Settings.Get().WinAiDbg.Debugging.SymbolSearchPath ?? string.Empty;
        var startupDelay = m_Settings.Get().WinAiDbg.Debugging.StartupDelayMs;
    ...
    {
        var installPath = m_Settings.Get().WinAiDbg.Service.InstallPath ?? string.Empty;
        var backupPath = m_Settings.Get().WinAiDbg.Service.BackupPath ?? string.Empty;
