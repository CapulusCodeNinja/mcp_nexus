...
namespace mcp_nexus.Resources
{
    ...
    public static class McpNexusResources
    {
        ...
        /// <summary>
        /// Lists all available extension scripts with their metadata, including name, description, version, and parameters.
        /// Extensions are PowerShell scripts that execute multiple debugging commands and implement sophisticated analysis patterns.
        /// </summary>
        /// <param name="serviceProvider">The service provider for dependency injection.</param>
        /// <returns>A JSON string containing all available extension scripts and their metadata.</returns>
        [McpServerResource, Description("🔌 EXTENSIONS: List all available extension scripts with metadata, parameters, and descriptions")]
        public static Task<string> Extensions(
            IServiceProvider serviceProvider)
        {
            var logger = serviceProvider.GetRequiredService<ILogger<Program>>();
            ...
            try
            {
                ...
                var result = new
                {
                    ...
                    usage = new
                    {
                        tool = "nexus_enqueue_async_extension_command",
                        description = "Use this tool to execute extensions",
                        example = new
                        {
            ...
            {
                logger.LogError(ex, "Error reading extensions list");
                throw;
            }
        }
            ...
            IServiceProvider serviceProvider)
        {
            var usage = new
            {
                ...
                available_resources = new[]
                {
                    ...
                    {
                        name = "extensions",
                        description = "🔌 EXTENSIONS: List all available extension scripts with metadata",
                        parameters = "None",
                        returns = "Array of extension information including name, description, version, author, and parameters"
                    },
                    new
                    {
                        name = "usage",
