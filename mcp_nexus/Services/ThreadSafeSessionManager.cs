...
namespace mcp_nexus.Services
{
    ...
    public class ThreadSafeSessionManager : ISessionManager, IDisposable
    {
            ...
            IOptions<CdbSessionOptions>? cdbOptions = null)
        {
            ...
            m_cleanupTimer = new Timer(async _ => await SafeCleanupExpiredSessions(), null, 
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                // LOCKING: Use dedicated lock for session creation (minimal scope)
                ...
                // ISOLATION: Create session components OUTSIDE of locks
                ...
                var startSuccess = await Task.Run(() => cdbSession.StartSession(cdbTarget, null));

                if (!startSuccess)
                {
                    await SafeDisposeCdbSession(cdbSession);
                ...
                // ATOMIC: Add to concurrent dictionary (thread-safe)
                if (!m_sessions.TryAdd(sessionId, newSession))
                {
                    // CLEANUP: Rare race condition - cleanup and retry
                    await SafeDisposeCdbSession(newSession.CdbSession);
                    newSession.CommandQueue.Dispose();
        ...
        public SessionContext GetSessionContext(string sessionId)
        {
            ...
            var queueStatus = session.CommandQueue.GetQueueStatus().ToList();
            var timeUntilExpiry = m_config.SessionTimeout - (DateTime.UtcNow - session.LastActivity);

            return new SessionContext
            {
                SessionId = sessionId,
                Description = $"Debugging session for {Path.GetFileName(session.DumpPath)}",
