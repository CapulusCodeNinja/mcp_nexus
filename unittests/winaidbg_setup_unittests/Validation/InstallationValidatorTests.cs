...
public class InstallationValidatorTests
{
    ...
    public void ValidateInstallation_WhenSourceExecutableNotExist_ReturnsFalse()
    {
        ...
        _ = m_FileSystemMock.Setup(fs => fs.DirectoryExists(sourceDirectory))
            .Returns(true);
        _ = m_FileSystemMock.Setup(fs => fs.FileExists(Path.Combine(sourceDirectory, "WinAiDbg.exe")))
            .Returns(false);
    ...
    public void ValidateInstallation_WhenBackupParentNotExist_ReturnsFalse()
    {
        ...
        // Backup path is @"C:\Program Files\WinAiDbg\backup"
        // The validation looks for "WinAiDbg" in the path and checks the parent, which is @"C:\Program Files"
    ...
    public void ValidateInstallation_WhenParentDirectoryPathIsEmpty_PassesValidation()
    {
        ...
        // Use a path that doesn't contain "WinAiDbg" so GetInstallationParentDirectory returns empty
    ...
    public void ValidateInstallation_WhenDirectoryPathParsingThrows_HandlesGracefully()
    {
        ...
        // Use a path with WinAiDbg that should trigger the FindIndex path
        var configWithWinAiDbgPath = new SharedConfiguration
        {
            WinAiDbg = new WinAiDbgSettings
            {
                Service = new ServiceSettings
        ...
        // Act
        var result = validator.ValidateInstallation(configWithWinAiDbgPath, sourceDirectory);
