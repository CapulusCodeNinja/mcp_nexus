...
namespace WinAiDbg.Config_unittests.Models;
...
public class SharedConfigurationTests
{
    ...
    public void SharedConfiguration_DefaultValues_ShouldBeCorrect()
    {
        ...
        _ = config.WinAiDbg.Should().NotBeNull();
        _ = config.WinAiDbg.Server.Should().NotBeNull();
        _ = config.WinAiDbg.Transport.Should().NotBeNull();
        _ = config.WinAiDbg.Debugging.Should().NotBeNull();
        _ = config.WinAiDbg.AutomatedRecovery.Should().NotBeNull();
        _ = config.WinAiDbg.Service.Should().NotBeNull();
        _ = config.WinAiDbg.SessionManagement.Should().NotBeNull();
        _ = config.WinAiDbg.Extensions.Should().NotBeNull();
        _ = config.WinAiDbg.Batching.Should().NotBeNull();
        _ = config.IpRateLimiting.Should().NotBeNull();
    }

    /// <summary>
    /// Tests that WinAiDbgSettings has correct default values.
    /// </summary>
    [Fact]
    public void WinAiDbgSettings_DefaultValues_ShouldBeCorrect()
    {
        // Act
        var settings = new WinAiDbgSettings();
    ...
    public void SharedConfiguration_CanBeModified_ShouldWork()
    {
        ...
        config.WinAiDbg.SessionManagement.MaxConcurrentSessions = 20;
        config.WinAiDbg.Debugging.CommandTimeoutMs = 600000;

        // Assert
        _ = config.Logging.LogLevel.Should().Be("Debug");
        _ = config.WinAiDbg.SessionManagement.MaxConcurrentSessions.Should().Be(20);
        _ = config.WinAiDbg.Debugging.CommandTimeoutMs.Should().Be(600000);
    }
    ...
    public void ServiceSettings_DefaultValues_ShouldBeCorrect()
    {
        ...
        // Assert
        _ = settings.InstallPath.Should().Be("C:\\Program Files\\WinAiDbg");
        _ = settings.BackupPath.Should().Be("C:\\Program Files\\WinAiDbg\\backups");
        _ = settings.ServiceName.Should().Be("WinAiDbg");
        _ = settings.DisplayName.Should().Be("WinAiDbg Debugging Server");
    }
