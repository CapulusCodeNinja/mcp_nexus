...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Tooling - Open Session** | `nexus_open_dump_analyze_session` | Open the analyze session for the dump file with the tool from Nexus MCP server. | `dumpFilePath` (string, required), `symbolsPath` (string, optional) | `sessionid` | This **EXACT sessionid IS REQUIRED TO BE USED** for all following commands in the session. |
