...
{
    private static readonly Regex WslPathRegex = new Regex(@"^/mnt/([a-zA-Z])(?:/(.*))?$", RegexOptions.Compiled);
    ...
    public static string ConvertToWindowsPath(string path)
    {
        ...
        try
        {
            ...
            if (match.Success)
            {
                ...
                return string.Format("{0}:\\{1}", driveLetter, restOfPath);
            }
        ...
        try
        {
            ...
            if (path.Length >= 2 && path[1] == ':' && char.IsLetter(path[0]))
            {
                ...
                return string.IsNullOrEmpty(restOfPath) ? 
                    string.Format("/mnt/{0}", driveLetter) : 
                    string.Format("/mnt/{0}/{1}", driveLetter, restOfPath);
            }
