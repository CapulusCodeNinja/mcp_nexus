...
namespace WinAiDbg.Setup.Validation
{
    ...
    internal class InstallationValidator : BaseValidator
    {
        ...
        public bool ValidateInstallation(SharedConfiguration configuration, string sourceDirectory)
        {
            ...
            if (!ValidateServiceNotInstalled(configuration.WinAiDbg.Service.ServiceName))
            {
                return false;
            }
            ...
            if (!ValidateTargetDirectoryPermissions(configuration.WinAiDbg.Service.InstallPath))
            {
                return false;
            }

            // Check 5: Backup directory permissions
            if (!ValidateBackupDirectoryPermissions(configuration.WinAiDbg.Service.BackupPath))
            {
                return false;
            }
        ...
        private bool ValidateSourceFiles(string sourceDirectory)
        {
            ...
            var sourceExecutablePath = Path.Combine(sourceDirectory, "WinAiDbg.exe");
