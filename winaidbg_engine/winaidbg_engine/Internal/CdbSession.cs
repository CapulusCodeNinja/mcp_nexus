...
internal class CdbSession : ICdbSession
{
    ...
    protected virtual Task WaitForProcessExitAsync()
    {
        if (m_CdbProcess != null && !m_CdbProcess.HasExited)
        {
            try
            {
                if (!m_CdbProcess.WaitForExit((int)m_Settings.Get().WinAiDbg.SessionManagement.GetCleanupInterval().TotalMilliseconds))
                {
    ...
    public Task<string> FindCdbExecutableAsync()
    {
        ...
        if (!string.IsNullOrEmpty(m_Settings.Get().WinAiDbg.Debugging.CdbPath) &&
            m_FileSystem.FileExists(m_Settings.Get().WinAiDbg.Debugging.CdbPath ?? string.Empty))
        {
            return Task.FromResult(m_Settings.Get().WinAiDbg.Debugging.CdbPath)!;
        }

        try
        {
            var locator = new WindowsKitsToolLocator(m_FileSystem);
            var resolved = locator.FindToolExecutablePath("cdb.exe", m_Settings.Get().WinAiDbg.Debugging.CdbPath, RuntimeInformation.OSArchitecture);
    ...
    private string GetCdbSessionBasedLogPath(string sessionId)
    {
        ...
        if (m_Settings.Get().WinAiDbg.Transport.ServiceMode)
        {
            // Service mode: C:\ProgramData\WinAiDbg\Sessions\
            sessionsDirectory = Path.Combine(Path.GetDirectoryName(directory)!, "Sessions");
        }
        else
        {
            // Other modes: C:\ProgramData\WinAiDbg\Logs\Sessions\
    ...
    {
        var timeout = TimeSpan.FromMilliseconds(m_Settings.Get().WinAiDbg.Debugging.StartupDelayMs);
