...
using WinAiDbg.Engine.Extensions;
...
public class DebugEngine : IDebugEngine
{
    ...
    internal DebugEngine(IFileSystem fileSystem, IFileCleanupQueue fileCleanupQueue, IProcessManager processManager, IBatchProcessor batchProcessor, ISettings settings)
    {
        ...
        m_Logger.Info("DebugEngine initialized with max {MaxSessions} concurrent sessions", m_Settings.Get().WinAiDbg.SessionManagement.MaxConcurrentSessions);

        var cleanupInterval = m_Settings.Get().WinAiDbg.SessionManagement.GetCleanupInterval();
    ...
    public async Task<CreateSessionResult> CreateSessionAsync(string dumpFilePath, string? symbolPath = null, CancellationToken cancellationToken = default)
    {
        ...
        if (m_Sessions.Count >= m_Settings.Get().WinAiDbg.SessionManagement.MaxConcurrentSessions)
        {
            throw new InvalidOperationException($"Maximum number of concurrent sessions ({m_Settings.Get().WinAiDbg.SessionManagement.MaxConcurrentSessions}) reached");
        }
    ...
    protected void CleanupIdleSessions()
    {
        ...
        var sessionTimeout = TimeSpan.FromMinutes(m_Settings.Get().WinAiDbg.SessionManagement.SessionTimeoutMinutes);
