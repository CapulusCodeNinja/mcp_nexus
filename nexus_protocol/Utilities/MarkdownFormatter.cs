...
internal static class MarkdownFormatter
{
    ...
    public static string GetUsageGuideMarkdown()
    {
        ...
        _ = md.AppendLine("- **Command status (get)**: Use `nexus_get_dump_analyze_commands_status` to poll ALL command statuses in a session (efficient for polling)");
        _ = md.AppendLine("- **Command results (read)**: Use `nexus_read_dump_analyze_command_result` to retrieve the FULL output for a single command; use the `commands` resource to browse summaries");
        _ = md.AppendLine("- **Sessions**: Opening a session does nothing by itself. Only open a session if you will enqueue commands.");
        _ = md.AppendLine("- **Commands**: After enqueueing, either poll with `nexus_get_dump_analyze_commands_status` (get = statuses) or fetch the full output with `nexus_read_dump_analyze_command_result` (read = full result). Enqueueing without monitoring or reading results is not useful.");
        _ = md.AppendLine();
        ...
        _ = md.AppendLine();
        _ = md.AppendLine("- **Tool name**: `nexus_get_dump_analyze_commands_status`");
        ...
        _ = md.AppendLine("- **Note**: Use this to efficiently monitor progress (get = poll statuses). For the full output of an individual command, use `nexus_read_dump_analyze_command_result` (read = full result)");
        _ = md.AppendLine();
