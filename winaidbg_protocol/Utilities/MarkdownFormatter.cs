...
internal static class MarkdownFormatter
{
    ...
    public static string GetUsageGuideMarkdown()
    {
        ...
        _ = md.AppendLine();
        _ = md.AppendLine("Complete guide for using the WinAiDbg MCP server tools and resources.");
        _ = md.AppendLine();
        _ = md.AppendLine("#### What WinAiDbg is");
        _ = md.AppendLine();
        _ = md.AppendLine("WinAiDbg is an MCP server for Windows crash dump analysis using WinDBG/CDB under the hood. You can use it to open `.dmp` files, run debugger/extension commands, and retrieve the output so an AI client can infer likely root causes and next steps.");
        _ = md.AppendLine("Think of it as a **bridge** that gives an AI client safe, structured access to Windows debugging tools such as `cdb.exe`/WinDBG for dump analysis.");
        _ = md.AppendLine();
        _ = md.AppendLine("- **Input**: Windows crash dumps (`.dmp`).");
        _ = md.AppendLine("- **What it does**: Opens a debugger session, executes commands asynchronously, and returns command statuses plus full command output.");
        _ = md.AppendLine("- **Why an AI client uses it**: Turn debugger evidence (exception codes, call stacks, modules/drivers, bucket IDs) into a concise root-cause narrative and actionable remediation steps.");
        _ = md.AppendLine();
        _ = md.AppendLine("#### Typical workflow");
        _ = md.AppendLine();
        _ = md.AppendLine("1. Open a session with `winaidbg_open_dump_analyze_session`.");
        _ = md.AppendLine("2. Enqueue commands with `winaidbg_enqueue_async_dump_analyze_command` (or `winaidbg_enqueue_async_extension_command`).");
        _ = md.AppendLine("3. Poll progress with `winaidbg_get_dump_analyze_commands_status` until completed.");
        _ = md.AppendLine("4. Fetch full output for specific commands via `winaidbg_read_dump_analyze_command_result`.");
        _ = md.AppendLine("5. Close the session with `winaidbg_close_dump_analyze_session`.");
        _ = md.AppendLine();
        _ = md.AppendLine("#### Output expectations (for AI clients)");
        _ = md.AppendLine();
        _ = md.AppendLine("- Separate **facts** (direct output) from **hypotheses** (inference).");
        _ = md.AppendLine("- Cite the **exact commands** you ran so results are reproducible.");
        _ = md.AppendLine("- If evidence is weak, propose **next commands** to reduce uncertainty.");
        _ = md.AppendLine();
